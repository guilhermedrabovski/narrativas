...
export default function App() {
  const [state, setState] = useState<EditorState>({
    ...
    logoSrc: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTHNKPX3z1yuRseNIZrS0jQpw7yTJ5EPVsiVWNg7V4oIA&s',
    logoY: -40,
    logoScale: 1,
    title: 'URGENTE!',
  ...
  useEffect(() => {
    const loadAsset = (url: string | null, ref: React.MutableRefObject<HTMLImageElement | null>) => {
      ...
      img.onerror = () => {
        console.warn(`Não foi possível carregar o asset: ${url}. Talvez bloqueio CORS ou o link não é uma imagem válida.`);
      };
    };
  ...
  const handleTimeUpdate = () => {
    if (videoRef.current) {
      ...
      // DESATIVADO durante exportação para evitar conflito
      if (!isExporting && isPlaying && state.endTime > 0 && v.currentTime >= state.endTime - 0.05) {
        v.currentTime = state.startTime;
      }
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.error("Failed to capture stream from canvas. Might be a CORS/Tainted Canvas issue.", e);
      alert("Erro ao exportar: Imagens bloqueadas por segurança (CORS). Tente usar suas próprias imagens do computador no momento.");
      setIsExporting(false);
      return;
    }
    ...
    recorder.onstop = async () => {
      ...
      // Aguarda 2 segundos antes de fechar o modal para que o usuário veja a conclusão (100%)
      setTimeout(() => {
        setIsExporting(false);
      }, 2000);
    };
    ...
    const interval = setInterval(() => {
      const isAtEnd = video.ended || video.currentTime >= state.endTime - 0.1;

      if (video.duration > 0 && isAtEnd) {
      ...
      <header className="border-b border-white/10 bg-black/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 overflow-hidden rounded-lg flex items-center justify-center shadow-lg shadow-red-900/20">
              <img 
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTHNKPX3z1yuRseNIZrS0jQpw7yTJ5EPVsiVWNg7V4oIA&s" 
                alt="Logo" 
