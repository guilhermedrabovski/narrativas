     ...
     {/* Left Column: Preview (Sticky) */}
        <div className="lg:col-span-4 sticky top-20 lg:top-24 z-40 flex flex-col gap-4 pointer-events-none lg:pointer-events-auto">
          <div className="relative aspect-[9/16] bg-black rounded-xl lg:rounded-3xl overflow-hidden border border-white/20 shadow-2xl ml-auto lg:mx-auto w-[120px] sm:w-[160px] lg:w-full max-w-[360px] pointer-events-auto transition-all">
            ...
            {/* Playback Controls Overlay */}
            {state.videoSrc && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/60 backdrop-blur-md px-6 py-3 rounded-full border border-white/10 opacity-0 hover:opacity-100 transition-opacity">
                <button onClick={togglePlay} className="p-2 hover:text-red-500 transition-colors">
                  {isPlaying ? <Pause className="w-6 h-6 fill-current" /> : <Play className="w-6 h-6 fill-current" />}
                </button>
                <button onClick={() => { if(videoRef.current) videoRef.current.currentTime = 0; }} className="p-2 hover:text-red-500 transition-colors">
                  <RotateCcw className="w-5 h-5" />
                </button>
                <div className="text-xs font-mono text-white/50 w-20 text-center">
                  {Math.floor(currentTime)}s / {Math.floor(duration)}s
                </div>
              </div>
            )}
